SELECT DISTINCT LOB_OBRA_V.obra_cod
	--,Doc_apdoc.PerPost
	,LOB_OBRA_V.obra_descripcion
	,IConstruye.dbo.ESTADOPAGO.NUMDOC
	,IConstruye.dbo.SUBCONTCONSOLIDADO.EMPVRAZONSOCIAL
	,convert(DATE, IConstruye.dbo.ESTADOPAGO.FECHAAPROBACION, 111) as FECHAAPROBACION
	,IConstruye.dbo.ESTADOSDOC.DESCRIPCIONC
	,LEFT(CONVERT(VARCHAR, convert(DATE, IConstruye.dbo.ESTADOPAGO.FECHACREACION, 111), 112), 6) as PERIODOCREACION
	,CASE 
		WHEN IConstruye.dbo.SUBCONTRATOS.RETIENEIVA = '0'
			THEN 'SI'
		ELSE 'NO'
		END AS RETIENEIVA
	,convert(DATE, IConstruye.dbo.ESTADOPAGO.FECHACREACION, 111) AS FECHACREACION
	...
	,LEFT(CONVERT(VARCHAR, convert(DATE, IConstruye.dbo.FACTURACOMPRADOR.FECHAEMISION, 111), 112), 6) AS FECHAEMISION
	,LOB_OBRA_V.emp_cod
	,IDCENTCOSTO
...
WHERE (Doc_apdoc.apdoc_type = 'D')
	AND (
		...
		--AND (Doc_apdoc.apdoc_type = 'D')
		AND LEFT(CONVERT(VARCHAR, convert(DATE, IConstruye.dbo.ESTADOPAGO.FECHACREACION, 111), 112), 6) = '201912'
